...
class DashboardsController extends AppController {
    ...
    public function certify() {
        error_reporting(0);
        if ($this->Session->check('udata1')) {
            $user_id = $this->Session->read('udata1');
            $data = $this->d_model->userData($user_id);
            $this->set('results', $data);
    ...
    public function cd_player() {
        ...
        if ($this->Session->check('udata1')) {
            ...
            $this->set('view_resume_data', $this->d_model->viewResumeSummaryData($user));
            if ($this->request->is('post')) {
...
<html>
   <head>
      <title>MyCareer Dashboard</title>
   </head>
   <body>
